  ...
  return (
    <div className="p-6 max-w-6xl">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3 mb-6">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">{t('ventas.title')}</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{t('ventas.subtitle')}</p>
        </div>
      ...
      <SectionAlerts tipo="ventas" refreshKey={refreshKey} />
      ...
      {noData && !error ? (
        <div className="p-8 bg-gray-50 dark:bg-gray-900 border border-dashed border-gray-300 dark:border-gray-700 rounded-xl text-center text-gray-500 dark:text-gray-400">
          <p className="text-4xl mb-3">💰</p>
          <p className="font-medium">{t('ventas.empty.sinDatos')}</p>
          <p className="text-sm mt-1">{t('ventas.empty.cargaCSV')}</p>
        </div>
      ) : (
        <>
          {/* KPIs */}
          <section className="mb-8">
            <SectionTitle>{t('ventas.sections.resumen')} · {periodLabel}</SectionTitle>
            <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
              <KpiMini title={t('ventas.kpi.totalVentas')} value={fmtCurrencyL(analytics.total_ventas)} icon="💰" color="green" />
              <KpiMini title={t('ventas.kpi.transacciones')} value={analytics.num_transacciones.toLocaleString()} icon="🧾" color="blue" />
              <KpiMini title={t('ventas.kpi.ticketPromedio')} value={fmtCurrencyL(analytics.ticket_promedio)} icon="📊" color="purple" />
              <KpiMini title={t('ventas.kpi.productoLider')} value={analytics.top_producto} subtitle={analytics.top_categoria} icon="⭐" color="amber" />
            </div>
          </section>
          ...
          <section className="mb-8 grid grid-cols-1 gap-6 lg:grid-cols-5">
            <div className="lg:col-span-3 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
              <div className="flex items-center justify-between mb-3">
                <SectionTitle>{t('ventas.sections.ventasPorGrupo', { groupBy: groupByLabel })} · {periodLabel}</SectionTitle>
                  ...
                  <div className="flex bg-gray-100 dark:bg-gray-700 rounded-lg p-0.5">
                    <button
                      ...
                      className={`px-2 py-1 text-xs font-medium rounded-md transition-colors cursor-pointer ${
                        chartType === 'area' ? 'bg-white dark:bg-gray-600 text-indigo-600 dark:text-indigo-400 shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'
                      }`}
                    ...
                    <button
                      ...
                      className={`px-2 py-1 text-xs font-medium rounded-md transition-colors cursor-pointer ${
                        chartType === 'bar' ? 'bg-white dark:bg-gray-600 text-indigo-600 dark:text-indigo-400 shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'
                      }`}
              ...
              {analytics.serie_temporal.length > 1 ? (
                <ResponsiveContainer width="100%" height={220}>
                  ...
                  <AreaChart data={analytics.serie_temporal} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                    ...
                    <Area type="monotone" dataKey="total" stroke="#6366f1" strokeWidth={2} fill="url(#gradVentas)" />
                  </AreaChart>
                  ) : (
                ...
                <p className="text-sm text-gray-400 dark:text-gray-500 mt-4">{t('ventas.empty.sinPuntos')}</p>
              )}
            </div>

            <div className="lg:col-span-2 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
              <SectionTitle>{t('ventas.sections.porCategoria')}</SectionTitle>
              {analytics.por_categoria.length > 0 ? (
                ...
                <p className="text-sm text-gray-400 dark:text-gray-500 mt-4">{t('ventas.empty.sinCategorias')}</p>
              )}
            </div>
          </section>
          ...
          {analytics.top_productos.length > 0 && (
            <section className="mb-8 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
              <SectionTitle>{t('ventas.sections.top5Productos')} · {periodLabel}</SectionTitle>
              <ResponsiveContainer width="100%" height={160}>
                <BarChart data={analytics.top_productos} layout="vertical" margin={{ top: 0, right: 16, left: 0, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke={chartGrid} horizontal={false} />
                  ...
                  <Tooltip formatter={v => [fmtCurrencyL(v), t('ventas.table.montoTotal')]} contentStyle={tooltipStyle} />
          ...
          <section>
            <SectionTitle>{t('ventas.sections.transaccionesRecientes')} · {periodLabel}</SectionTitle>
            <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
                    <th className="text-left px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide">{t('ventas.table.fecha')}</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide">{t('ventas.table.producto')}</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide">{t('ventas.table.categoria')}</th>
                    <th className="text-right px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide">{t('ventas.table.cantidad')}</th>
                    <th className="text-right px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide">{t('ventas.table.monto')}</th>
                  </tr>
                </thead>
                <tbody>
                  {transacciones.map((v, i) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-2.5 text-gray-800 dark:text-gray-200 font-medium max-w-48 truncate">
                        {v.nombre_producto}
                      </td>
                      <td className="px-4 py-2.5">
                        {v.categoria ? (
                          <span className="text-xs bg-indigo-50 dark:bg-indigo-900 text-indigo-700 dark:text-indigo-400 px-2 py-0.5 rounded-full">
                            {v.categoria}
                          </span>
                        ) : (
                      ...
                      <td className="px-4 py-2.5 text-right text-gray-600 dark:text-gray-300">{v.cantidad}</td>
                      <td className="px-4 py-2.5 text-right font-semibold text-gray-900 dark:text-gray-100">
                        {fmtCurrencyL(v.monto_total)}
                      </td>
                    </tr>
                  ))}
                </tbody>
              ...
              {analytics.num_transacciones > 15 && (
                <p className="px-4 py-2 text-xs text-gray-400 dark:text-gray-500 border-t border-gray-100 dark:border-gray-800">
                  {t('ventas.mostrando', { total: analytics.num_transacciones.toLocaleString() })}
                </p>
              )}
            </div>
          </section>
